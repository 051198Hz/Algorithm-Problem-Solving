...
for _ in 0..<file.readInt(){
    let priceNumber = file.readInt()
    let stikerNumber = file.readInt()
    var prices: [[Int]] = []
    var totalPrice = 0
    for _ in 0..<priceNumber{
        let price: [Int] = file.readIntArray(file.readInt()+1)
        prices.append(price)
    }
    let stikers:[Int] = file.readIntArray(stikerNumber)
    for price in prices {
        var minNumber = Int.max
        for i in price.indices where i < price.count - 1{
            minNumber = min(minNumber, stikers[price[i]-1])
        }
        totalPrice += (minNumber * price.last!)
    }
    print(totalPrice)
}
