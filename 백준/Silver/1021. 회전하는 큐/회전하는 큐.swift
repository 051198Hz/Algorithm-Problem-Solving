...
var targets:[Int] = file.readStringArray(count).map{ Int($0)! }
var q = Deque(sequence: Array(1...size))
var answer = 0
for target in targets{
    while(true){
        ...
        var idx = 0
        while(true){
            if target == tmpq.front! {break}
            tmpq.pop_front()
            idx += 1
        }
        let leftDistace = q.size - idx
        let rightDistance = idx
        if leftDistace < rightDistance{
            answer += leftDistace
            for _ in 0..<leftDistace{
                q.push_front(q.pop_back()!)
            }
            if q.pop_front()! == target {break}
            else { assert(q.front! != target, "target is not consistent with front")}
        }else{
