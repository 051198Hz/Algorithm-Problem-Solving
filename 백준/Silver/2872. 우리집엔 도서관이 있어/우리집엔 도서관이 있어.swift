...
var file = FileIO()
var bookNumber = file.readInt()
var books: [Int] = []
var count = 0
var max = Int.min
var maxIndex = 0
for i in 0..<bookNumber{
    let book = file.readInt()
    books.append(book)
    if max < book{
        max = book
        maxIndex = i
...
var descendingBooks = 0
for i in stride(from: maxIndex, to: -1, by:-1){
    if books[i] == max{
        max -= 1
        descendingBooks += 1
    }
}
let answer = bookNumber - descendingBooks
print(answer)
