...
if number == 1{
    ...
    let prices = file.readIntArray(number).sorted(by: >)
    var answer = 0
    var count = 0
    for i in stride(from: 0, to: number - (number%3),by: 1){
        count += 1
        if count%3 != 0{ answer += prices[i]}
    }
    for i in stride(from: number-1, to: number-1-(number%3), by: -1) {
        answer += prices[i]
    }
    print(answer)
}
