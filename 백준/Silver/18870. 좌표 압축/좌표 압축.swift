...
// String.contains(String) == self.range(of: other) != nil
var file = FileIO()
var dict = [Int:Int]()
let length = file.readInt()
var poses = file.readIntArray(length)
var encode = 0
//minPos가 10^-9, maxPos가 10^9이므로 탐색 범위에서 시간초과.
for i in poses.sorted(){
    if dict[i] == nil{
        dict.updateValue(encode, forKey: i)
        encode += 1
    }
}

print("\(poses.map{ String(dict[$0]!) }.joined(separator: " "))")


//poses.forEach{ print("\(dict[$0]!)",terminator: " ") }
