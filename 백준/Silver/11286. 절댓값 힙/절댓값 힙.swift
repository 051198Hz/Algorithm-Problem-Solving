...
var file = FileIO()
var absoluHeap = Heap<AbsoluteValue>(<)
var answer = ""
for _ in 0..<file.readInt(){
    let n = AbsoluteValue(origin: file.readInt())
    switch n.origin{
    case 0: answer += "\(absoluHeap.delete()?.origin ?? 0 )\n"
    default: absoluHeap.insert(element: n)
    }
}
print(answer)

struct AbsoluteValue: Comparable{
    static func < (lhs: AbsoluteValue, rhs: AbsoluteValue) -> Bool {
        if lhs.abs == rhs.abs {
    ...
    init(origin: Int){
        self.origin = origin
        abs = Swift.abs(origin)
    }
}
//> 최대힙
//> 최소힙
//compare을 채택할 때는 >를 구현해야 합니다
