...
let push_front = stringToAscii("push_front")
let push_back = stringToAscii("push_back")
let pop_front = stringToAscii("pop_front")
let pop_back = stringToAscii("pop_back")
...
for _ in 0..<n {
    ...
    switch cmd {
    case push_front:
        q.push_front(file.readInt())
    case push_back:
        q.push_back(file.readInt())
    case pop_front: //pop
        answer += "\(q.pop_front() ?? -1)\n"
    case pop_back: //pop
        answer += "\(q.pop_back() ?? -1)\n"
    case size: //size
        answer += "\(q.size)\n"
