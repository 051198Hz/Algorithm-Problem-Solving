...
var file = FileIO()
//입력받은 떡국 그릇을 내림차순으로 정렬한다.
//탑 배열을 선언하고, 각 탑마다 현재 쌓을 수 있는 그릇 크기를 저장한다.
// 어느 탑에도 현재 인덱스의 그릇을 쌓을 수 없다면, 새 그릇을 쌓는다.

// 크기가 겹치는 그릇이 있다면, 해당 탑에는 더이상 쌓을 수 없다.
// 따라서 새 탑으로 옮겨야 한다.
//겹치는 그릇의 크기 갯수?

//겹치는 그릇 갯수 중 제일 큰 갯수만큼 탑이 생긴다.
// 1 1 1 2 2 3
// 3
...
let plateNumber = file.readInt()
var plateDupleCount = [Int](repeating: 0, count: 50000 + 1)
var towerNumber = 1
for _ in 0..<plateNumber{
    let plate = file.readInt()
    plateDupleCount[plate] += 1
}
print(plateDupleCount.max()!)
