...
let times = file.readInt()
var answer = ""
for _ in 0..<times{
    let points = file.readInt()

    var dp1:[Int] = [file.readInt()]
    var dp2:[Int] = [file.readInt()]

    let s1shoot = file.readInt()
    let s2shoot = file.readInt()
    ...
    for _ in 0..<points-1{ s1pass.append(file.readInt()) }
    for _ in 0..<points-1{ s1drible.append(file.readInt()) }
    for _ in 0..<points-1{ s2pass.append(file.readInt()) }
    for _ in 0..<points-1{ s2drible.append(file.readInt()) }
    
    for i in 0..<points-1{
        //각각의 dp는 현재 스트라이커가 자신이 공을 가지고 있는 최저 난이도이다.
        //스트라이커는 전달받은 공을 드리블 할수도 있고, 다른 스트라이커로부터 패스받을수도 있다.
        dp1.append(min(dp1[i] + s1drible[i], dp2[i] + s2pass[i])) //자신의 공을 드리블 or 다른 스트라이커로부터 공 패스받음
        dp2.append(min(dp2[i] + s2drible[i], dp1[i] + s1pass[i]))
    }
    answer += "\(min(dp1.last! + s1shoot, dp2.last! + s2shoot))\n"
}
print(answer)
