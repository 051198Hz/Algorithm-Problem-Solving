...
var file = FileIO()
let cityNumber = file.readInt()
var distances: [UInt64] = []
for _ in 0..<cityNumber - 1{
    distances.append(file.readInt().toUInt64)
}
var currentPrice:UInt64 = 0
var answer: UInt64 = 0
for i in 0..<cityNumber-1{
    let newPrice = file.readInt().toUInt64
    if i == 0 || newPrice < currentPrice{ //해당 도시가 이전 도시보다 기름값이 비싸면, 이전 도시에의 기름값을 주유함
