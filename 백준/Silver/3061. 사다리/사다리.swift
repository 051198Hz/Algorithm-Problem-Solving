...
for _ in 0..<time{
    var count = 0
    var ladderNumber = file.readInt()
    var ladders = [Int]()
    for _ in 0..<ladderNumber{
        ladders.append(file.readInt() - 1)
    }
    for i in 0..<ladderNumber{
        if i != ladders[i]{
            var index = -1
            for j in i+1..<ladderNumber{
                if ladders[j] == i{
                    index = j
                    break
                }
            }
            for j in stride(from: index, to: i, by: -1){
                ladders.swapAt(j-1, j)
                count += 1
            }
        }
    }
