...
let topNumber = file.readInt()

let dowPrice = file.readInt()
let topingPrice = file.readInt()
let dowCal = file.readInt()


var currentCal = dowCal
var currentPrice = dowPrice

var topCals = file.readIntArray(topNumber).sorted(by: >)

for topCal in topCals{
    let tmpCal = currentCal + topCal
    let tmpPrice = currentPrice + topingPrice
    if Double(currentCal/currentPrice) > Double(tmpCal/tmpPrice){ break }
    currentPrice += topingPrice
    currentCal += topCal
}

print( currentCal / currentPrice )
